...
class Food{
    ...
    class Square{
        constructor(x,y){
            ...
            this.width = 10;
            this.height = 10;
            this.back = null; //cola de la snake
        }
    ...
    window.addEventListener("keydown", function(event) {
        if(event.keyCode == 83) return snake.down();
        if(event.keyCode == 68) return snake.right();
        if(event.keyCode == 87) return snake.up();
        if(event.keyCode == 65) return snake.left();
    })
